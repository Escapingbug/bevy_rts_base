...
fn update_camera_position(
    ...
    has_camera_query: Query<(&CanHaveCamera, &Transform), Without<Camera>>,
) {
...
impl Plugin for CameraPlugin {
    fn build(&self, app: &mut App) {
        app.add_system(update_camera_position)
            .add_system(switch_camera_to_entity)
            .add_system(reset_unit_target_if_it_has_camera)
            .add_system(switch_after_current_unit_dies)
            .add_system(switch_camera_back);
    }
}
