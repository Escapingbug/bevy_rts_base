...
fn create_drag_rectangle(
    ...
    //mut _color_materials: ResMut<Assets<ColorMaterial>>,
) {
    ...
    commands
        .spawn_bundle(PbrBundle {
            ...
            material: materials.add(StandardMaterial {
                base_color: Color::rgba(0.0, 1.0, 0.8, 0.1),
                unlit: true,
                ..default()
            }),
...
fn drag_select(
    ...
    pick_state_query: Query<&PickingCamera>,
    mut meshes: ResMut<Assets<Mesh>>,
    mut drag_selection_rectangle: Query<(&Handle<Mesh>, &DragSelectionRectangle, &mut Visibility)>,
) {
    if ability.ability != Ability::Select && ability.ability != Ability::HealArea {
        return;
    }
    ...
    if let Some(initial_position) = selection_state.initial_position {
        if let Some((_top_entity, intersection)) = pick_state.intersect_top() {
            let current_position = Vec3::from(intersection.position());
